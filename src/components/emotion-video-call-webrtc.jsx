...
const EmotionVideoCallWithWebRTC = () => {
  ...
  const getIceServers = async () => {
    ...
    if (meteredApiKey) {
      try {
        console.log('🔄 Attempting Metered.ca TURN servers...');
        ...
        if (response.ok) {
          ...
          console.log('✅ Using Metered.ca TURN servers:', servers.length);
          return { 
            iceServers: servers,
        ...
        console.warn('⚠️ Metered.ca unavailable:', error.message);
      }
    }

    // Comprehensive fallback with free public TURN servers
    console.log('📡 Using enhanced fallback ICE configuration');
    ...
    const config = {
      ...
      // Try all connection methods (host, srflx, relay)
      iceTransportPolicy: 'all',
    ...
    console.log(`   Total servers: ${config.iceServers.length}`);
    console.log(`   STUN servers: ${config.iceServers.length - turnCount}`);
    console.log(`   TURN servers: ${turnCount}`);
    
    if (!meteredApiKey) {
      addAlert('Using free TURN servers - suitable for testing', 'info');
    } else {
  ...
  const createPeerConnection = async (remotePeerId, stream) => {
    ...
    peerConnection.onicecandidate = (event) => {
      if (event.candidate) {
        ...
        // Parse candidate details for better debugging
        let candidateType = 'unknown';
        let protocol = 'unknown';
        
        if (c.candidate) {
          if (c.candidate.includes('typ host')) candidateType = 'host';
          else if (c.candidate.includes('typ srflx')) candidateType = 'srflx (STUN)';
          else if (c.candidate.includes('typ relay')) candidateType = 'relay (TURN)';
          else if (c.candidate.includes('typ prflx')) candidateType = 'prflx';
          
          if (c.candidate.includes('udp')) protocol = 'udp';
          else if (c.candidate.includes('tcp')) protocol = 'tcp';
        }
        
        console.log(`🧊 Local ICE candidate #${localCandidateCount}: ${candidateType} ${protocol}`);
        console.log('   Address:', c.address || 'N/A');
        console.log('   Port:', c.port || 'N/A');
        console.log('   Priority:', c.priority || 'N/A');
        
        // Highlight TURN candidates (critical for NAT traversal)
        if (candidateType === 'relay (TURN)') {
          console.log('✅ TURN candidate found - excellent for NAT traversal!');
        }
        ...
        console.log(`   Total candidates gathered: ${localCandidateCount}`);
      }
    };
    ...
    peerConnection.oniceconnectionstatechange = () => {
      ...
      switch (state) {
        case 'new':
          console.log('🆕 ICE agent initialized');
          break;
          
        case 'checking':
          console.log('🔍 Testing connectivity between peers...');
          addAlert('Establishing connection...', 'info');
          break;
          
        case 'connected':
          console.log('✅ ICE connected! Peer-to-peer link established');
          addAlert('Connected successfully!', 'success');
          setIsConnected(true);
          
          // Log selected candidate pair after connection
          setTimeout(() => {
            peerConnection.getStats().then(stats => {
              stats.forEach(report => {
        ...
        case 'completed':
          console.log('✅ ICE completed - optimal path selected and stable');
          setIsConnected(true);
          break;
        ...
        case 'failed':
          ...
          console.log(`   Remote candidates: ${remoteCandidateCount}`);
          console.log(`   Metered key: ${meteredApiKey ? 'SET' : 'NOT SET'}`);
          
          addAlert('Connection failed. Attempting to recover...', 'alert');
          ...
          try {
            if (peerConnection.restartIce) {
              ...
              console.warn('⚠️ ICE restart not supported, may need to recreate connection');
            }
          } catch (error) {
        ...
        case 'disconnected':
          console.warn('⚠️ ICE disconnected - connection interrupted');
          ...
          reconnectTimeoutRef.current = setTimeout(() => {
            if (peerConnection.iceConnectionState === 'disconnected') {
              console.log('🔄 Connection still down, triggering ICE restart');
              if (peerConnection.restartIce) {
                peerConnection.restartIce();
              }
            }
        ...
        case 'closed':
          console.log('🔒 ICE connection closed normally');
          setIsConnected(false);
          break;
      ...
      // Original diagnostic logging for disconnected/failed states
